...
class sale_quote(http.Controller):
    ...
    @http.route(["/quote/substitute_line/<int:line_id>/<int:order_id>/<token>"], type='http', auth="public", website=True)
    def substitute_line(self, line_id, order_id, token, **post):
        ...
        if Order.state not in ('draft', 'sent'):
            return False
        LineSub = request.env['sale.order.substitute'].sudo().browse(int(line_id))
        # request.env['sale.order.line'].sudo().browse(int(line_id))
        LineSub.button_substitute_product()
        return werkzeug.utils.redirect("/quote/%s/%s#quote_header_2" % (Order.id, token))
